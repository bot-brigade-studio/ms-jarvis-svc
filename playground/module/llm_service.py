...
class StreamResponse:
    def __init__(self, content: str, index: int, is_final: bool = False, finish_reason: Optional[str] = None):
...
class LLMService:
    ...
    async def generate_stream(
        ...
        try:
            if config.provider == ProviderType.ANTHROPIC:
                async for chunk in self._stream_anthropic_response(messages, config, system_prompt):
                    yield chunk
            else:
                async for chunk in self._stream_openai_response(messages, config, system_prompt):
                    yield chunk
