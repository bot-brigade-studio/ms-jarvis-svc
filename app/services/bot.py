...
class BotService:
    ...
    async def create_bot(self, schema: BotCreate) -> Bot:
        is_exists = await self.bot_repo.get(filters={"name": schema.name}, select_fields=["id"])
        if is_exists:
    ...
    async def create_bot_config(self, bot_id: UUID, schema: BotConfigCreate) -> BotConfig:
        bot_exists = await self.bot_repo.get(filters={"id": bot_id}, select_fields=["id"])
        if not bot_exists:
            raise APIError(status_code=404, message="Bot not found")
    ...
    async def update_bot_config(self, bot_id: UUID, config_id: UUID, schema: BotConfigCreate) -> BotConfig:
        bot_config = await self.bot_config_repo.get(filters={"id": config_id, "bot_id": bot_id}, select_fields=["id"])
        if not bot_config:
            raise APIError(status_code=404, message="Bot config not found")
        
        await self.config_variable_repo.delete(filters={"config_id": bot_config.id}, force=True)
    ...
    async def save_config_variables(self, config: BotConfig):
        ...
        if custom_instructions:
            ...
            for variable in variables:
                await self.config_variable_repo.create({"key": variable, "value": None, "config_id": config.id})
        else:
            await self.config_variable_repo.delete(filters={"config_id": config.id}, force=True)
    ...
    async def get_bots(self, skip: int = 0, limit: int = 10, filters: Dict[str, Any] = None):
        return await self.bot_repo.get_multi(skip=skip, limit=limit, filters=filters)
    
    async def get_bot(self, id: UUID):
        return await self.bot_repo.get(filters={"id": id}, load_options=["configs.variables"])
