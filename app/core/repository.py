...
class BaseRepository(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def get(
        ...
        """
        query = self._build_query(load_options=load_options, select_fields=select_fields, is_tenant_scoped=is_tenant_scoped, with_deleted=with_deleted)
    ...
    async def get_by(
        ...
        query = self._build_query(filters=filters, joins=joins, load_options=load_options, select_fields=select_fields, is_tenant_scoped=is_tenant_scoped, with_deleted=with_deleted)
        result = await self.db.execute(query)
        return result.unique().scalar_one_or_none()
